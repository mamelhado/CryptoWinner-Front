...
const seriesData = new Map([
    ...
    useEffect(() => {
        ...
        const fetchHistory = async (): Promise<void> => {
            const response = await fetch(`${REST_URL_BASE}?symbol=${chartSymbol}&interval=${chartInterval}`);
        ...
        if (!newConnection){
            ...
            if(newConnection.state == signalR.HubConnectionState.Connected)
                await newConnection.invoke("Subscribe", `${chartSymbol}`, `${chartInterval}`);
        ...
        return () => {
            destroyed = true;
            //ws?.close();
            if(historyChartSymbol.length > 1)
            {
                console.log("close hub", `${historyChartSymbol[historyChartSymbol.length - 1]}`)
                connection?.invoke("Unsubscribe", `${historyChartSymbol[historyChartSymbol.length - 1]}`, `${chartInterval}`);
    ...
    return (
        <div 
            ...
            <Legend 
                ...
                symbols={symbols}
                onChange={(e) => {
                    setHistoryChartSymbol([...historyChartSymbol, chartSymbol]);
                    setChartSymbol(e);
                }}
            />
            <div
